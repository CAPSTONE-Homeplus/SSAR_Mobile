...
void main() {
  ...
  test('should return transaction list when repository call is successful', () async {
    // Arrange
    when(mockRepository.getTransactionByUserWallet(walletId, search, orderBy, page, size))
        .thenAnswer((_) async => fakeResponse);

    // Act
    final result = await useCase.call(walletId, search, orderBy, page, size);

    // Assert
    expect(result, Right(fakeResponse));
    verify(mockRepository.getTransactionByUserWallet(walletId, search, orderBy, page, size)).called(1);
