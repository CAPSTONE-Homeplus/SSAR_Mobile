...
class AppSignalrService {
  ...
  final OrderTrackingLocalDataSource _orderTrackingLocalDataSource;
  ...
  // Public streams
  Stream<NotificationModel> get walletNotificationStream => _walletNotificationController.stream;
  Stream<OrderTracking> get orderTrackingNotificationStream => _orderTrackingNotificationController.stream;
  Stream<LaundryOrderToUser> get laundryOrderNotificationStream => _laundryOrderToUserNotificationController.stream;
  ...
  static Future<AppSignalrService> init({
    ...
    required OrderTrackingLocalDataSource orderTrackingLocalDataSource,
  }) async {
    if (_instance == null) {
      _instance = AppSignalrService._internal(
        authLocalDataSource: authLocalDataSource,
        orderTrackingLocalDataSource: orderTrackingLocalDataSource,
      );
  ...
  AppSignalrService._internal({
    ...
    required OrderTrackingLocalDataSource orderTrackingLocalDataSource,
        ...
        _orderTrackingLocalDataSource = orderTrackingLocalDataSource;
  ...
  Future<void> _initVinWalletHub() async {
    ...
    _vinWalletHubConnection.on('ReceiveNotificationToAll', (arguments) => _handleInviteWalletNotification(arguments, 'VinWallet'));
    _vinWalletHubConnection.on('ReceiveNotificationToUser', (arguments) => _handleInviteWalletNotification(arguments, 'VinWallet'));
    _vinWalletHubConnection.on('ReceiveNotificationToGroup', (arguments) => _handleInviteWalletNotification(arguments, 'VinWallet'));
  }
  ...
  void _handleInviteWalletNotification(Object? arguments, String source) {
    ...
    _showLocalNotification('Ví', message, 'wallet');
  }
      ...
      if (jsonData.containsKey('Type') && jsonData.containsKey('Data')) {
        ...
        if (type == 'OrderTracking') {
          ...
          if (data is Map<String, dynamic>) {
            ...
            await _orderTrackingLocalDataSource.saveOrderTracking(orderTracking);
            _orderTrackingNotificationController.add(orderTracking);
            _showLocalNotification('Theo dõi đơn hàng', 'Đơn hàng ${orderTracking.orderId} đã được cập nhật', 'order_tracking');
          }
        }
      } else {
      ...
      if (jsonData.containsKey('Type') && jsonData.containsKey('Data')) {
        ...
        if (type == 'LaundryOrderToUser') {
          ...
          if (data is Map<String, dynamic>) {
            ...
            print('📢 Nhận thông báo đơn giặt sấy từ $source: ${laundryOrderToUser.orderCode}');

            _laundryOrderToUserNotificationController.add(laundryOrderToUser);
            _showLocalNotification('Đơn giặt ', 'Đơn hàng ${laundryOrderToUser.orderCode} đã được cập nhật', 'laundry_order');
          }
        }
      } else {
        // Trường hợp không có wrapper, thử parse trực tiếp
        ...
        _laundryOrderToUserNotificationController.add(laundryOrderToUser);
        _showLocalNotification('Đơn giặt sấy', 'Đơn hàng ${laundryOrderToUser.orderCode} đã được cập nhật trang thái ${LaundryOrderStatusExtension.fromString(laundryOrderToUser.status?.toLowerCase() ?? '')}', 'laundry_order');
      }
    } catch (e, stackTrace) {
  ...
  Future<String> _getAccessToken() async {
    String? currentToken = await _authLocalDataSource.getAccessTokenFromStorage();
    return currentToken ?? '';
  }
